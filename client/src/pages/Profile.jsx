...
const Profile = () => {
  ...
  <div className="relative min-h-screen pt-24 py-10 px-4 flex flex-col items-center overflow-hidden bg-cover bg-center" style={{ backgroundImage: "url('/assets/loginbk1.jpg')" }}>
    ...
    <div className="relative z-10 bg-white/80 dark:bg-[#000000] shadow-lg rounded-xl p-8 w-full max-w-3xl">
      ...
      <div className="flex items-center space-x-6 mb-8">
        <div className="relative">
          <img
            src={profilePic}
            alt="Profile"
            className="w-28 h-28 object-cover rounded-full border-4 border-purple-200 dark:border-purple-500"
          />
          <label
        ...
        <div>
          <h2 className="text-2xl font-semibold text-[#431363] dark:text-purple-300">Your Profile</h2>
          <p className="text-gray-500 dark:text-gray-400">Edit your details below</p>
        </div>
      </div>
      ...
      <form className="space-y-4">
        <div>
          ...
          <input
            ...
            className="w-full mt-1 px-4 py-2 border rounded-md focus:ring-2 focus:ring-purple-400 dark:bg-[#000000] dark:border-gray-600 dark:text-white dark:placeholder-gray-400"
        ...
        <div>
          ...
          <input
            ...
            value={email}
            disabled
            className="w-full mt-1 px-4 py-2 border rounded-md bg-gray-100 dark:bg-gray-800 dark:border-gray-600 cursor-not-allowed text-gray-500 dark:text-gray-300"
          />
        </div>
        ...
        <div className="flex gap-4">
          <div className="w-1/2">
            ...
            <input
              ...
              className="w-full mt-1 px-4 py-2 border rounded-md focus:ring-2 focus:ring-purple-400 dark:bg-[#000000] dark:border-gray-600 dark:text-white"
            />
          </div>
        ...
        <div>
          <label className="block text-gray-700 dark:text-gray-300">Short Bio</label>
          <textarea
            value={bio}
        ...
        <div className="flex justify-between mt-6">
          <button
            type="button"
            onClick={handleSave}
            className="bg-[#431363] text-white px-6 py-2 rounded-md hover:bg-[#5e1d91] dark:bg-purple-700 dark:hover:bg-purple-800 transition"
          >
            Save Changes
          </button>
          <button
      ...
      <hr className="my-10 border-t-2 border-purple-300 dark:border-purple-600" />

      {/* User's posted products */}
      <div className="mt-10">
        <h3 className="text-xl font-semibold text-[#431363] dark:text-purple-300 mb-4">Your Posted Products</h3>

        {loadingProducts ? (
          <p className="text-gray-600 dark:text-gray-400">Loading your products...</p>
        ) : userProducts.length === 0 ? (
          <p className="text-gray-500 dark:text-gray-400 italic">You have not posted any products yet.</p>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {userProducts.map((product) => (
              <div
                key={product._id}
                className="bg-white dark:bg-[#000000] rounded-lg shadow-md border border-[#e2e4ed] dark:border-gray-600 p-4 flex gap-4"
              >
                <img
                ...
                <div className="flex flex-col justify-between">
                  <div>
                    <h4 className="font-bold text-lg text-[#431363] dark:text-purple-300">{product.title}</h4>
                    <p className="text-gray-600 dark:text-gray-400 text-sm line-clamp-2">{product.description}</p>
                    <p className="mt-1 text-[#431363] dark:text-purple-300 font-semibold">${product.price}</p>
                  </div>
                  <p className="text-xs mt-2 text-gray-500 dark:text-gray-400">
                    Status:{' '}
                    <span className={product.isApproved ? 'text-green-600 dark:text-green-400' : 'text-yellow-600 dark:text-yellow-400'}>
      ...
      <div className="mt-10">
        <button
          ...
          className="bg-purple-700 text-white px-6 py-2 rounded-md hover:bg-purple-800 dark:bg-purple-600 dark:hover:bg-purple-700 transition"
        >
          Post Product
        </button>
      </div>
